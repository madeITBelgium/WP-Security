...
class WP_MadeIT_Security_Admin {
    ...
    public function initMenu() {
        ...
        add_menu_page(__('Security & Maintenance', 'madeit_security'), __('Security', 'madeit_security') . " " . $new, 'manage_options', 'madeit_security', array($this, 'show_dashboard'), MADEIT_SECURITY_URL . 'assets/icon-16x16.png', 9999);
        add_submenu_page('madeit_security', __('Security Dashboard', 'madeit_security'), __('Dashboard', 'madeit_security'), 'manage_options', 'madeit_security', array($this, 'show_dashboard'));
        add_submenu_page('madeit_security', __('Security Scan', 'madeit_security'), __('Scan', 'madeit_security'), 'manage_options', 'madeit_security_scan', array($this, 'show_scan'));
        add_submenu_page('madeit_security', __('Security Settings', 'madeit_security'), __('Settings', 'madeit_security'), 'manage_options', 'madeit_security_settings', array($this, 'settings'));
    }

    public function initStyle() {
        wp_register_style('madeit-security-admin-style', MADEIT_SECURITY_URL . 'admin/css/style.css', array(), null);
        wp_enqueue_style('madeit-security-admin-style');
        
        wp_register_style('madeit-tabs', MADEIT_SECURITY_URL . 'admin/css/tabs.css', array(), null);
        wp_enqueue_style('madeit-tabs');
        wp_register_style('madeit-grid', MADEIT_SECURITY_URL . 'admin/css/grid.css', array(), null);
        wp_enqueue_style('madeit-grid');
        wp_register_style('madeit-card', MADEIT_SECURITY_URL . 'admin/css/card.css', array(), null);
        wp_enqueue_style('madeit-card');
        
        wp_register_style('font-awesome', 'https://netdna.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.css', array(), null);
        wp_enqueue_style('font-awesome');

        wp_enqueue_script('jquery-ui-core');
        wp_enqueue_script('jquery-ui-tabs');

        wp_enqueue_script('madeit-security-script', MADEIT_SECURITY_URL . 'admin/js/script.js', array('jquery'), 1, true);
        wp_enqueue_script('madeit-tabs', MADEIT_SECURITY_URL . 'admin/js/tabs.js', array('jquery'), 1, true);
    }
    ...
    private function save_settings() {
        ...
        if (!wp_verify_nonce($nonce, 'madeit_security_settings')) {
            ...
            else if(isset($checkApiKey['success']) && $checkApiKey['success']) {
                $this->defaultSettings = $this->settings->loadDefaultSettings();
    ...
    private function timeAgo($ptime) {
        ...
        $a = array(12 * 30 * 24 * 60 * 60 =>  __('year', 'madeit_security'),
    ...
    public function addHooks() {
        ...
        add_action('wp_ajax_madeit_security_repo_scan', array($this, 'doRepoScan'));
        add_action('wp_ajax_madeit_security_update_scan', array($this, 'doUpdateScan'));
        add_action('wp_ajax_madeit_security_backup', array($this, 'doBackup'));
