...
class WP_MadeIT_Security_Firewall
{
    ...
    private function installFirewall($optionFound, $option, $optionName, $backupFiles, $currentAutoPrependFile, $autoPrependHelper)
    {
        ...
        if ($optionFound) {
            ...
            if (false === ($credentials = request_filesystem_credentials($ajaxURL, '', false, ABSPATH, ['version', 'locale', 'action', 'serverConfiguration', 'currentAutoPrepend'], $allow_relaxed_file_ownership))) {
            ...
            try {
                ...
                $verifyURL = add_query_arg(['firewall_action' => 'install_status'], $ajaxURL);
                $response = wp_remote_get($verifyURL, ['headers' => ['Referer' => false]]);
    ...
    private function uninstallFirewall($optionFound, $option, $optionName, $backupFiles, $currentAutoPrependFile, $autoPrependHelper)
    {
        ...
        $allow_relaxed_file_ownership = true;
        ob_start();
        if (false === ($credentials = request_filesystem_credentials($ajaxURL, '', false, ABSPATH, ['version', 'locale', 'action', 'serverConfiguration', 'iniModified'], $allow_relaxed_file_ownership))) {
            $credentialsContent = ob_get_clean();
        ...
        try {
            if ($autoPrependHelper->usesUserIni($option) && !isset($_GET['remove_ini'])) { //Uses .user.ini but not yet modified
                ...
                } elseif (!$autoPrependHelper->usesUserIni($option)) {
